...
final class SupervisionIdentityStore {
    static let directory: URL = {
        ...
        return support.appendingPathComponent("Cable/SupervisionIdentity", isDirectory: true)
    }()
    ...
    /// The app was called Bottle before it was called Cable. Anyone who set up a
    /// phone with that build has their key under the old folder; move it across
    /// once so they don't have to re-import it.
    private static func migrateFromOldAppName() {
        let fm = FileManager.default
        let support = fm.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
        let old = support.appendingPathComponent("Bottle", isDirectory: true)
        let new = support.appendingPathComponent("Cable", isDirectory: true)
    ...
    private static func load() -> SupervisionIdentity? {
        ...
        return SupervisionIdentity(
            ...
            organizationName: org?.isEmpty == false ? org! : "Cable"
        )
    }
    ...
    private func extract(p12: URL, password: String, log: ActivityLog) async throws {
        ...
        let env = ["CABLE_P12_PASSWORD": password]
        try await openssl(["pkcs12", "-in", p12.path, "-passin", "env:CABLE_P12_PASSWORD", "-clcerts", "-nokeys", "-out", certPEM.path], environment: env, log: log)
        try await openssl(["pkcs12", "-in", p12.path, "-passin", "env:CABLE_P12_PASSWORD", "-nocerts", "-nodes", "-out", keyPEM.path], environment: env, log: log)
        try await openssl(["x509", "-in", certPEM.path, "-outform", "DER", "-out", Self.certificateURL.path], log: log)
        try await openssl(["rsa", "-in", keyPEM.path, "-outform", "DER", "-out", Self.privateKeyURL.path], log: log)
    }
