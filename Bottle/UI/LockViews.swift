...
/// Locking is the one irreversible-feeling thing Bottle does, so it gets a
...
struct LockSheet: View {
    ...
    private var afterLockExplainer: some View {
        VStack(alignment: .leading, spacing: 7) {
            ...
            bullet("iphone", "You unblock from the iPhone itself — Bottle gives you a link to keep on its Home Screen.")
            bullet("laptopcomputer.slash", "This Mac can't change the block any more, even if you reinstall Bottle.")
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            if let approver = created.approverUrl {
                ...
                VStack(alignment: .leading, spacing: 9) {
                    ...
                    Text("Bottle won't show it again — copy it somewhere safe or send it now.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
            ...
            VStack(alignment: .leading, spacing: 22) {
                if let record = lock.record {
                    ...
                    if let error = lock.errorMessage {
                        NoticeBanner(tone: .warning, title: "Couldn't reach Bottle", detail: error,
        ...
        case "unlocking":
            card(tint: .orange) {
                VStack(alignment: .leading, spacing: 11) {
                    ...
                    Text("You don't have to keep this open — the wait runs on Bottle's side.")
                        .font(.callout)
                        .foregroundStyle(.secondary)
