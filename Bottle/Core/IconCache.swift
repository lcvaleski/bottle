...
/// cached per device under ~/Library/Caches/Bottle/Icons/<UDID>/.
@MainActor
@Observable
final class IconCache {
    static let root: URL = {
        let caches = FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask)[0]
        return caches.appendingPathComponent("Bottle/Icons", isDirectory: true)
    }()
    ...
    func ensureIcons(for bundleIDs: [String], ecid: String, deviceKey: String) async {
        ...
        let scratch = FileManager.default.temporaryDirectory
            .appendingPathComponent("bottle-icons-\(UUID().uuidString)", isDirectory: true)
